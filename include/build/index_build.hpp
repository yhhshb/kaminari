...
{
    //STEP 1 : PARSE FILES =====================================================
    if (build_parameters.verbose >= 1) std::cout << "[I] Step 1 & 2: parsing " << nb_docs << " files and merging results" << std::endl;
    ...
    {
        if (build_parameters.verbose >= 1) std::cout << "[I] Step 3: building the MPHF for " << nb_elems << " minimizers" << std::endl;
    ...
    {
        if (build_parameters.verbose >= 1) std::cout << "[I] Step 4.1: colors deduplication and storing: ColorSets" << std::endl;
        start_time = std::chrono::high_resolution_clock::now();
    ...
    //STEP 5 : COLORMAPPER  =============================================
    {
        if (build_parameters.verbose >= 1) std::cout << "[I] Step 4.2: mapping hash -> minimizer: ColorMapper" << std::endl;
        start_time = std::chrono::high_resolution_clock::now(); 
